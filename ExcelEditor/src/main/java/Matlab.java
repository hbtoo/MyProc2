...
public class Matlab {
    ...
    public Matlab(String sourceDir, String destDir) {
    ...
    /**
     * run matlab start_km_server.m, which generates R_t.xlsx and t.xlsx
     * update the values into matlab.xlsx -> matlab-out.xlsx
     * return selected rows in matlab-out.xlsx
     */
    public List<List<Double>> runMatlabAndGenExcel(String params, double r0, boolean skipMatlab) {
        if (!skipMatlab) {
            try {
                File outputExcel = new File(sourceDir + "/R_t.xlsx");
                outputExcel.delete();
                outputExcel = new File(sourceDir + "/t.xlsx");
                outputExcel.delete();
                ...
                ProcessBuilder pb = new ProcessBuilder(
                        ...
                        "-r",
                        //"-batch",
                        "run('" + sourceDir + "/start_km_server.m')"   // command inside MATLAB
                );
        ...
        List<Map.Entry<Integer, List<Double>>> tValues =
                getColumnValues(sourceDir + "/t.xlsx", List.of(0), 0, false);
        List<Map.Entry<Integer, List<Double>>> r_tValues =
                getColumnValues(sourceDir + "/R_t.xlsx", List.of(0), 0, false);

        if (!updateMatlabExcel(tValues, r_tValues, r0)) {
            return null;
        }
    ...
    /**
     * matlab.xlsx -> matlab-out.xlsx
     */
    private boolean updateMatlabExcel(
            List<Map.Entry<Integer, List<Double>>> tValues,
            List<Map.Entry<Integer, List<Double>>> r_tValues,
            double r0) {
        try (FileInputStream fis = new FileInputStream(sourceDir + "/matlab.xlsx");
            ...
            for (Row row : sheet) {
                ...
                if (n - beginRow >= tValues.size()) {
                    System.out.println("not enough t, r_t rows from matlab output until line "
                ...
                // override R(M) with calculated value, using R0 from UI
                cell = row.getCell(2);
                if (cell == null) cell = row.createCell(2);
                cell.setCellValue(r0 * r_tValues.get(n - beginRow).getValue().get(0));
            ...
            try (FileOutputStream fos = new FileOutputStream(destDir + "/matlab-out.xlsx")) {
                workbook.write(fos);
            }
            return true;
        } catch (Exception ex) {
    ...
    public List<List<Double>> getTKLines() {
        List<Map.Entry<Integer, List<Double>>> tkValues = getColumnValues(destDir + "/matlab-out.xlsx",
